...
//C-RAM alarm Code://////////////////////////////////////////
#define BUZZER_PIN A0 // Example pin
...
SoftwareSerial SerialNicla(4, 7); //(RX, TX) so Nicla TX1 -> RX 'D4' and Nicla RX1 -> TX 'D7'



void setup() {
  Serial.begin(19200);     // USB serial for debug
  SerialNicla.begin(19200);  // RX/TX connected to Nicla Vision

  reportEvent("STATUS,STARTING");
...
void loop() {
  ...
  if (SerialNicla.available()) { // is camera Identifies Threat
    ...
    Serial.println("Received from Nicla: " + data);
    ...
    if (firstComma > 0 && secondComma > firstComma) {
      ...
      if (getSavedPosition(tempX, tempY, tempDis)){
        ...
        reportEvent(
          String("POSITION_CHANGE,") +
          changeInPos.x + "," +
          changeInPos.y + "," +
          changeInPos.distance_mm + "," +
          String(totalChange, 2)
        );
      ...
      // SEND RESPONSE BACK TO NICLA
      SerialNicla.print("ACK,");
      SerialNicla.println(data);

      Serial.print("Sent to Nicla: ACK,");
      Serial.println(data);

      savePosition(xPix,yPix,distance_mm);
      reportEvent("MOTOR,Y,1000");
      moveSteps(stepperY, 1000);
      reportEvent("MOTOR,X,1000");
      moveSteps(stepperX, 1000);
      //delay(10000); //Stuck forever until remove this line. I dont want it to repeat a ton of times and damage the C-RAM.
    }
  ...
  bool yWasMoving = (stepperY.distanceToGo() != 0);
  bool xWasMoving = (stepperX.distanceToGo() != 0);

  stepperY.run();
  stepperX.run();

  bool yIsMoving = (stepperY.distanceToGo() != 0);
  bool xIsMoving = (stepperX.distanceToGo() != 0);

  // Y finished
  if (yWasMoving && !yIsMoving) {
    reportEvent(
      String("MOTOR_DONE,Y,") +
...
void moveSteps(AccelStepper &stepper, long steps) {
    long target = stepper.currentPosition() + steps;
    stepper.moveTo(target);   // just sets the target
}
...
// ============================================================
// Nano -> Nicla Event Reporting
// ============================================================

void reportEvent(String event)
{
  // Send event to Nicla
  SerialNicla.println(event);

  // Also display event on Nano USB Serial Monitor
  Serial.print("EVENT -> Nicla: ");
