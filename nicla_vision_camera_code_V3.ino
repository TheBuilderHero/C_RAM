...
#include "gc2145.h"
#include <ea_malloc.h>
#include "VL53L1X.h" //proximity sensor.
...
//SoftwareSerial Serial1(10 , 9); // RX, TX pins
...
void setup()
{
    ...
    Serial.begin(19200); // Initialize the USB serial port for debugging
    NICLA_HW_SERIAL.begin(19200);
    ...
    /* Setup the distance sensor code so that we can read the distance data from the sensor.*/
    Wire1.begin();
    Wire1.setClock(400000);           // Fast I2C
    ...
    proximity.startContinuous(50);    // Read every 50 ms
}
...
void loop()
{
    ...
    if (ei_sleep(2000) != EI_IMPULSE_OK) { // wait 2 seconds
        return;
    }
    ...
    for (uint32_t i = 0; i < result.bounding_boxes_count; i++) {
        ...
        uint16_t objectDistance_mm = proximity.read();
        Serial.print("Distance (mm): ");
        Serial.println(objectDistance_mm);

        if (objectDistance_mm == 0 || objectDistance_mm > 3900) { // Adjust threshold for your application
            Serial.println("Out of range");
            objectDistance_mm = 9999; // Use a sentinel value for 'too far away'
        }

        // Calculate center coordinates
        uint16_t center_x = bb.x + (bb.width / 2);
        uint16_t center_y = bb.y + (bb.height / 2);

        // Debug print over USB
        ei_printf("Object: %s (%f) Center: (%u, %u) Distance: %u mm\r\n",
                bb.label, bb.value, center_x, center_y, objectDistance_mm);

        // Send to Arduino Uno over NICLA_HW_SERIAL in CSV format
        NICLA_HW_SERIAL.print(center_x);
        NICLA_HW_SERIAL.print(",");
        NICLA_HW_SERIAL.print(center_y);
        NICLA_HW_SERIAL.print(",");
        NICLA_HW_SERIAL.println(objectDistance_mm);
...
void checkNanoResponse()
{
    while (NICLA_HW_SERIAL.available())
    {
        String response = NICLA_HW_SERIAL.readStringUntil('\n');
        response.trim();
